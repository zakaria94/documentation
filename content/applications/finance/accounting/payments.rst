...
In Odoo, payments can either be automatically linked to an invoice or bill; or be stand-alone
...
journal entry and changes the amount due according to the amount of the payment. The counterpart is
reflected in an :ref:`outstanding receipts <bank/outstanding-accounts>` or **outstanding payments**
account. At this point, the customer invoice or vendor bill is marked as :guilabel:`In payment`.
Then, when the outstanding account is reconciled with a bank statement line, the invoice or vendor
...
.. tip::
   - If your main bank account is set as :ref:`outstanding account
     ...
     bank statement), invoices and bills are directly registered in the :guilabel:`Paid` status.
...
are matched with the **outstanding account** until they are manually matched with their
...
list view of payments, you can select several payments and group them in a batch by clicking
...
allows you to process them all one by one, matching all payments and invoices ine one place. You can
...
:menuselection:`Accounting --> Reconciliation`.
...
If you wish to register a **partial payment**, click :guilabel:`Register payment` from the related
invoice or bill, and enter the amount received or paid. Upon entering the amount, a prompt appears
to decide whether to :guilabel:`Keep open` the invoice or bill, or :guilabel:`Mark as fully paid`.
Select :guilabel:`Keep open` and click :guilabel:`Create payment`. The invoice or bill is now marked
as :guilabel:`Partial`. Select :guilabel:`Mark as fully paid` if you wish to settle the invoice or
bill with an amount difference.
...
next step is to :doc:`reconcile <bank/reconciliation>` it with the related bank statement line to
