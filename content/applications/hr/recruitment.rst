...
.. image:: recruitment/add-column.png
   :align: center
   :alt: THe plus sign to click in order to add a new column to the kanbn stages.
...
- :guilabel:`Show in Referrals`: Check the box if this stage should be seen in the *Referrals*
  ...
  If this is active, a :guilabel:`Points` field appears. Enter the amount of referral points
...
- :guilabel:`Recruiter`: Select the person responsible for the entire recruitment process for the
  job position. Only *user's* can be selected, and all user's are presented in the drop-down to
  select from.
...
The *Calendar* application loads, showing the currently scheduled meetings and events for the user.
The meetings and events displayed are for the employees who are listed under the
